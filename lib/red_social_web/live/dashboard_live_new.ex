defmodule RedSocialWeb.DashboardLive do
  ...
  def handle_event("select_user", %{"user_id" => user_id}, socket) do
    ...
    following = SocialCore.get_followers(user)
  ...
  def handle_event("follow_user", %{"user_id" => user_id}, socket) do
    ...
    case SocialCore.follow(current_user, user_to_follow) do
      {:ok, _relationship} ->
        # Refresh data
        following = SocialCore.get_followers(current_user)
        feed = apply_feed_filter(current_user, socket.assigns.feed_filter)
    ...
    <div class="min-h-screen bg-gradient-to-br from-blue-50 to-purple-50 p-6">
      <div class="max-w-[1800px] mx-auto">
        ...
        <div class="mb-8">
          ...
          <p class="text-gray-600 font-medium">Simulate user interactions and explore the social network</p>
        </div>
          ...
          <div class="space-y-6">
            <%= if @current_user_id do %>
              ...
              <div class="bg-white rounded-xl shadow-lg border border-gray-200 p-6">
                <h3 class="font-bold text-gray-900 mb-4 text-lg flex items-center gap-2">
                  <.icon name="hero-pencil-square" class="w-5 h-5 text-blue-600" />
                  Create Post
                </h3>
              ...
              <div class="space-y-4">
                <h3 class="font-bold text-gray-900 text-xl flex items-center gap-2">
                  <.icon name="hero-newspaper" class="w-6 h-6 text-blue-600" />
                  Feed
                ...
                <div class="space-y-4 max-h-[800px] overflow-y-auto pr-2">
                  <%= for post <- @feed do %>
                    <div class="bg-white border-2 border-gray-200 p-6 rounded-xl shadow-lg hover:shadow-2xl transition-all hover:border-blue-300 transform hover:scale-[1.01]">
                      ...
                      <!-- Post Content -->
                      <p class="text-gray-800 mb-4 text-base leading-relaxed font-medium">{post.content}</p>
                      ...
                      <div class="flex items-center gap-6 text-sm text-gray-600 mb-4 pb-4 border-b-2 border-gray-100">
                        <span class="font-bold flex items-center gap-1">
                          ...
                          {Enum.count(post.interactions, fn i -> i.type == "like" end)}
                        </span>
                        <span class="font-bold flex items-center gap-1">
                          <.icon name="hero-hand-thumb-down" class="w-4 h-4 text-gray-500" />
                          {Enum.count(post.interactions, fn i -> i.type == "dislike" end)}
                        </span>
                        <span class="font-bold flex items-center gap-1">
                          <.icon name="hero-arrow-path" class="w-4 h-4 text-blue-500" />
                          {Enum.count(post.interactions, fn i -> i.type == "repost" end)}
                        </span>
                      </div>
            ...
            <% else %>
              <div class="bg-white rounded-xl shadow-lg border border-gray-200 p-12 text-center">
                ...
                <p class="text-gray-600">Choose a user from the left sidebar to simulate their experience</p>
              </div>
            <% end %>
          </div>
                        ...
                        <div class="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <p class="text-xs text-gray-600 font-medium">
                              {if user.type == "company", do: "🏢", else: "👤"} {String.capitalize(user.type)}
                            </p>
                          </div>
                        </div>
                      </div>
